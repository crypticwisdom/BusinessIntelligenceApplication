...
def transaction_status_per_channel(user, inst_id, channel, duration):
    ...
    result = secondary_cache.get(cache_key)
    if result:
        return result
    ...
    secondary_cache.set(cache_key, result, timeout=cache_timeout)
...
def total_admin_count_report(user, inst_id):
    ...
    if result is None:
        ...
        secondary_cache.set(key=cache_key, value=result, timeout=cache_timeout)
...
#     if secondary_cache.get(cache_key):
#         result = secondary_cache.get(cache_key)
#     else:
#         result = dict()
#         week = 0
...
#     if (result := secondary_cache.get(cache_key)) is not None:
#         return result
