...
def ir(data):
    ...
    if volume:
        combined_hits: list = combined_hits[: int(volume)]
        total = volume
    return True, combined_hits, int(total)
...
def issuer_filter(data, name, acquire_id, type) -> dict:
    ...
    if False:
        ...
        if not processing == "others":
            upbin = ExtraParameters.objects.filter(name="UPBIN").first() or []

        totalbin: set = set(bin).union(set(acquireBin)).union(set(upbin))
        ...
        if schema:
            schema_filters = [
                Q("regexp", pan={"value": f"{value}.*"}) for value in schema
            ]
            q &= Q("bool", should=schema_filters, minimum_should_match=1)
        if issuer_country:
            q &= Q("terms", issuer_country=[str(value) for value in issuer_country])
        if account_currency:
            q &= Q("terms", account_currency=[str(value) for value in account_currency])
        if currency:

            q &= Q("terms", currency=[str(value) for value in totalCurrency])
        if curreny_orig:
            q &= Q("terms", curreny_orig=[str(value) for value in curreny_orig])
        if channel:
            channel_value = []
            if type == "bespoke":
        ...
        if pan:
            q &= Q("term", pan=pan)
        if totalbin:
            bin_filter = [Q("prefix", pan=value) for value in totalbin]
        ...
        transaction_s = TransactionsDocument.search().query(
            q
            & Q("term", department=type)
            & Q("range", transaction_time={"gte": start_date, "lt": end_date})
        )

        response = transaction_s.params(scroll="2m").execute()
        ...
        return True, all_hits, total_hits
